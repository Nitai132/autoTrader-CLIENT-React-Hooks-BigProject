...
const ChooseSymbolsModal = ({ currentAccount, userEmail, setUserSymbols, financialTech, unSavedChangesFlag }) => {
  ...
  useEffect(async () => {
    if (userEmail.length > 0) {
      ...
      let dataRows = []
      data.map((row) => {
        dataRows.push(createData(Object.values(row)[1], Object.values(row)[2], Object.values(row)[3], Object.values(row)[4], Object.values(row)[5]))
      });
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <p style={{ fontSize: '20px' }}>
            You can search for the symbol you would like to find
          </p>
          <p style={{ fontSize: '17px' }}>
            You can <a 
            href="https://docs.google.com/spreadsheets/d/1qa4SeBf9oQTjChKYWtwkAknjaGaA9cmLlQx8Vbl6ylc/edit#gid=0"
            target="blank"
            >click here</a> to see all the symbols and the default symbols
          </p>
